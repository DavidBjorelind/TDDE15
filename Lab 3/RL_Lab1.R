...
for(j in c(0.5,0.75,0.95)){ # Using different GAMMAS, EPS = 0.5
  q_table <- array(0,dim = c(H,W,4))
  reward <- NULL
  correction <- NULL
  ...
  title = paste("Reward: Gamma = ",j, ", Epsilon = 0.5" )
  plot(MovingAverage(reward,100),type = "l", main = title)
  title = paste("Correction: Gamma = ",j, ", Epsilon = 0.5" )
  plot(MovingAverage(correction,100),type = "l", main = title)
  print(length(reward[which(reward==10)]))
}

for(j in c(0.5,0.75,0.95)){ # Using different GAMMAS, EPS = 0.1
  q_table <- array(0,dim = c(H,W,4))
  reward <- NULL
  correction <- NULL
  ...
  title = paste("Reward: Gamma = ",j, ", Epsilon = 0.1" )
  plot(MovingAverage(reward,100),type = "l", main=title)
  title = paste("Correction: Gamma = ",j, ", Epsilon = 0.1" )
  plot(MovingAverage(correction,100),type = "l", main=title)
  print(length(reward[which(reward==10)]))
}
...
for(j in c(0,0.2,0.4,0.66)){ # Different BETAS, EPS = 0.5 (default)
  q_table <- array(0,dim = c(H,W,4))
